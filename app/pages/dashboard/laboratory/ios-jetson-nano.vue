...
const isCamFullscreen = ref(false)
...
const checkServiceStatus = async (url: string, type: 'vnc' | 'camera') => {
  ...
  try {
    ...
    if (type === 'vnc') {
      ...
      toast.add({ title: '✅ VM disponible', color: 'success' })
    } else {
      servicesStatus.value.camera = true
      camError.value = false
      toast.add({ title: '✅ Cámara disponible', color: 'success' })
    }
  } catch (error) {
    if (type === 'vnc') {
      servicesStatus.value.vnc = false
...
const checkAllServices = async () => {
  ...
  toast.add({ title: '🔍 Verificando servicios...', color: 'info' })
  await Promise.all([
...
const onVncLoad = () => {
  ...
  vncError.value = false
  servicesStatus.value.vnc = true
  toast.add({ title: 'Escritorio conectado', color: 'success' })
}
...
const capturePhoto = () => {
  toast.add({ title: 'Capturando foto...', color: 'info' })
  setTimeout(() => {
...
const recordScreen = () => {
  toast.add({ title: 'Iniciando grabación...', color: 'info' })
  setTimeout(() => {
    toast.add({ title: 'Grabación iniciada', color: 'success' })
  }, 1000)
}
    ...
    <template #body>
      <div class="p-2 min-h-full">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-2 mb-2">
          <!-- Máquina Virtual -->
          <UCard class="overflow-hidden p-0 shadow-sm rounded-md">
            <template #header>
            ...
            <div class="relative bg-black" style="height: 55vh;">
              ...
              <!-- Error de conexión -->
              <div v-else-if="vncError" class="absolute inset-0 flex flex-col items-center justify-center bg-gray-900 z-20">
          ...
          <UCard id="cam-card" class="overflow-hidden p-0 shadow-sm rounded-md">
            ...
            <div class="relative bg-black" style="height: 55vh;">
              ...
              <!-- Loading -->
              <div v-else-if="camLoading" class="absolute inset-0 flex items-center justify-center bg-black/90 z-20">
                <div class="text-center">
              ...
              <!-- iframe -->
              <iframe v-if="camUrl" ref="camIframe" :src="camUrl" class="w-full h-full relative z-10" frameborder="0"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-2">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div
                  ...
                  <div
                    ...
                    <div v-if="servicesStatus.camera" class="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></div>
          ...
          <UCard class="p-0 shadow-sm rounded-md">
            <template #header>
              <div class="flex items-center gap-1.5">
                <UIcon name="i-lucide-info" class="w-3 h-3 text-purple-500" />
                <span class="text-[10px] font-bold uppercase tracking-wide">Info Lab</span>
              </div>
            </template>

            <div class="p-3">
              <p class="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                Este laboratorio remoto permite controlar arduino, esp32, sensores y componentes electrónicos en tiempo
